...
describe('NoveltyMatrixService Unit Tests', () => {
  ...
  let mockDeconstructionService: any;
  ...
  beforeEach(() => {
    ...
    noveltyMatrixService = new NoveltyMatrixService(
      mockSearchService,
      mockLLMProvider,
      mockDeconstructionService
    );
  ...
  describe('NoveltyMatrixPromptBuilder Unit Tests', () => {
    it('should normalize status codes correctly', () => {
      const fallback = NoveltyMatrixPromptBuilder.createHeuristicFallback(
        [{ id: 'F1', name: 'solar cell', description: 'photovoltaic' }],
        [{ patentId: 'PAT-1', title: 'Solar cell charger', abstract: 'photovoltaic solar cell charger', score: 0.9 }]
      );

...
describe('FeatureDeconstructionService Unit Tests', () => {
  ...
  let service: FeatureDeconstructionService;
...
\`\`\``;
  ...
  describe('FeatureDeconstructionPromptBuilder Unit Tests', () => {
    ...
    it('createFallbackResult should extract features from multi-sentence text', () => {
      const fallback = FeatureDeconstructionPromptBuilder.createFallbackResult(
        'First sentence describing solar panel array. Second sentence describing MPPT controller. Third sentence describing battery backup unit.'
      );

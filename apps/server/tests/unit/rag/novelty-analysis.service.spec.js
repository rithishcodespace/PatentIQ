...
describe('NoveltyAnalysisService Unit Tests', () => {
    ...
    beforeEach(() => {
        ...
        noveltyAnalysisService = new NoveltyAnalysisService(mockSearchService, mockLLMProvider, mockHistoryService);
    });
    describe('Query & Request Validation', () => {
        ...
        it('should throw BadRequestError if topK is invalid (< 1 or > 100)', async () => {
            await expect(noveltyAnalysisService.analyzeNovelty({ query: mockSearchQuery, topK: 0 })).rejects.toThrow(BadRequestError);
            await expect(noveltyAnalysisService.analyzeNovelty({ query: mockSearchQuery, topK: 150 })).rejects.toThrow(BadRequestError);
        });
    });
    describe('Novelty Analysis Execution & LLM Parsing', () => {
        ...
        it('should propagate ServiceUnavailableError when LLM provider fails', async () => {
            mockLLMProvider.generateCompletion.mockRejectedValueOnce(new ServiceUnavailableError('Ollama Qwen model unavailable'));
            await expect(noveltyAnalysisService.analyzeNovelty({ query: mockSearchQuery })).rejects.toThrow(ServiceUnavailableError);
        });
    });
});
//# sourceMappingURL=novelty-analysis.service.spec.js.map
...
import { mockSearchQuery, mockSearchRequest, mockVectorEmbedding, mockPineconeMatches, } from '../../fixtures/search.fixtures.js';
describe('SearchService Unit Tests', () => {
    ...
    beforeEach(() => {
        ...
        searchService = new SearchService(mockEmbeddingProvider, mockSearchRepository, mockHistoryService, undefined, mockCacheProvider);
    });
    describe('Query Validation & Embedding Generation', () => {
        ...
        it('should propagate ServiceUnavailableError when Ollama embedding fails', async () => {
            mockEmbeddingProvider.generateEmbedding.mockRejectedValueOnce(new ServiceUnavailableError('Ollama service unreachable'));
            await expect(searchService.generateEmbedding(mockSearchQuery)).rejects.toThrow(ServiceUnavailableError);
        });
    });
    describe('Pinecone Search & Ranking', () => {
        it('should invoke Pinecone search with query vector, topK, and filters', async () => {
            ...
            expect(mockSearchRepository.querySimilarity).toHaveBeenCalledWith(mockVectorEmbedding, 5, { ipc: { $in: ['B64C 39/02'] } });
        });

...
describe('EmbeddingsService Unit Tests', () => {
  ...
  const validDocument: StandardPatentDocument = {
  ...
  describe('generatePatentDocumentEmbeddings', () => {
    ...
    it('should throw BadRequestError if document is null or undefined', async () => {
      await expect(
        embeddingsService.generatePatentDocumentEmbeddings(null as any)
      ).rejects.toThrow(BadRequestError);
    });

    it('should throw BadRequestError if all section texts are empty', async () => {
      const emptyDoc: StandardPatentDocument = {
        title: '   ',
        abstract: '',
        claims: '  ',
        keywords: [],
      ...
      await expect(
        embeddingsService.generatePatentDocumentEmbeddings(emptyDoc)
      ).rejects.toThrow('contains no valid text sections');
    });

    it('should propagate ServiceUnavailableError when Ollama service is unreachable', async () => {
      (mockEmbeddingProvider.generateBatchEmbeddings as any).mockRejectedValueOnce(
        new ServiceUnavailableError('Ollama service unavailable')
      );

      await expect(
        embeddingsService.generatePatentDocumentEmbeddings(validDocument)
      ).rejects.toThrow(ServiceUnavailableError);
    });

    it('should propagate InternalServerError when Ollama API returns invalid response', async () => {
      (mockEmbeddingProvider.generateBatchEmbeddings as any).mockRejectedValueOnce(
        new InternalServerError('Received empty response from Ollama')
      );

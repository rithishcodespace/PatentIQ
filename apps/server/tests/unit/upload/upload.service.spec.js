...
describe('UploadService Unit Tests', () => {
    ...
    describe('Successful Document Uploads', () => {
        it('should successfully process and save a valid PDF document', async () => {
            ...
            expect(mockUploadRepository.create).toHaveBeenCalledWith(expect.objectContaining({
                originalFileName: 'Patent_Application.pdf',
                mimeType: 'application/pdf',
                extension: 'pdf',
    ...
    describe('Validation Failures', () => {
        it('should throw BadRequestError when payload or file buffer is missing', async () => {
            await expect(uploadService.uploadDocument(null)).rejects.toThrow(BadRequestError);
            await expect(uploadService.uploadDocument({ filename: 'test.pdf', mimetype: 'application/pdf', buffer: null })).rejects.toThrow(BadRequestError);
        });
        it('should throw BadRequestError for empty (0 bytes) file', async () => {
            const emptyBuffer = Buffer.alloc(0);
            await expect(uploadService.uploadDocument({
                filename: 'empty.pdf',
                mimetype: 'application/pdf',
                buffer: emptyBuffer,

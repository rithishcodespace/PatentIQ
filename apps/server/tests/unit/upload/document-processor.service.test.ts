...
import type { IDocumentParser } from '../../../src/modules/upload/interfaces/upload-processor.interface.js';

describe('DocumentProcessorService Unit Tests', () => {
  let service: DocumentProcessorService;
  let mockPdfParser: IDocumentParser;
  let mockDocxParser: IDocumentParser;
  let mockTxtParser: IDocumentParser;
  ...
  describe('processFile', () => {
    ...
    it('should throw BadRequestError on extremely short extracted text', async () => {
      (mockTxtParser.parse as any).mockResolvedValueOnce({ bodyText: 'Short' });
  ...
  describe('processDirectText', () => {
    ...
    it('should throw BadRequestError if title is missing', async () => {
      ...
      await expect(service.processDirectText(input as any)).rejects.toThrow('Patent title is required');
    });
    ...
    it('should throw BadRequestError if abstract is missing', async () => {
      ...
      await expect(service.processDirectText(input as any)).rejects.toThrow('Patent abstract is required');
    });
    ...
    it('should throw BadRequestError if claims are missing', async () => {
      ...
      await expect(service.processDirectText(input as any)).rejects.toThrow('Patent claims');
    });
    ...
    it('should throw BadRequestError if total text is extremely short', async () => {
      ...
      await expect(service.processDirectText(input as any)).rejects.toThrow('extremely short');
    });
  });
});

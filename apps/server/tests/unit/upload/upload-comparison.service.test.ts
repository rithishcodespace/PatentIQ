...
describe('UploadComparisonService Unit Tests', () => {
  let service: UploadComparisonService;
  ...
  beforeEach(() => {
    ...
    service = new UploadComparisonService(
      mockUploadService,
      mockDocumentProcessorService,
      mockEmbeddingsService,
      mockSearchService,
  ...
  it('should throw NotFoundError if documentId does not exist', async () => {
    ...
    await expect(
      service.compareDocument({ documentId: 'invalid-id' })
    ).rejects.toThrow(NotFoundError);
  });

  it('should propagate embedding generation failure', async () => {
    mockEmbeddingsService.generatePatentDocumentEmbeddings.mockRejectedValueOnce(
      new InternalServerError('Ollama model error')
    );

    await expect(
      service.compareDocument({ document: validDocument })

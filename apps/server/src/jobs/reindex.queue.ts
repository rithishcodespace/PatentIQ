...
export class ReindexQueue {
  private queue: Queue<ReindexJobData> | null = null;
  private worker: Worker<ReindexJobData> | null = null;
  private connection: any = null;
  private isRedisAvailable = false;
  private prisma: PrismaClient;
  private embeddingsService?: EmbeddingsService | undefined;
  ...
  constructor(
    ...
    try {
      ...
      this.queue = new Queue<ReindexJobData>('reindex-embeddings-queue', {
        connection: this.connection,
      });

      this.worker = new Worker<ReindexJobData>(
        'reindex-embeddings-queue',
   ...
   */
  async addReindexJob(data: { forceAll: boolean; batchSize: number }): Promise<{ jobId: string; queuedAt: Date }> {
   ...
   */
  private async processReindexJob(data: ReindexJobData): Promise<void> {

...
export class HistoryService implements IHistoryService {
   ...
   */
  async saveSearchHistory(dto: CreateSearchHistoryDto): Promise<any> {
    ...
    try {
      ...
      console.log(
        `[HistoryService] Search saved | id=${record.id} | query="${record.searchQuery}" | resultsCount=${dto.totalResults} | dbLatency=${dbLatency}ms`
      );
   ...
   */
  async saveNoveltyAnalysis(dto: CreateNoveltyAnalysisDto): Promise<any> {
    ...
    try {
      ...
      console.log(
        `[HistoryService] Analysis saved | searchHistoryId=${dto.searchHistoryId} | confidenceScore=${dto.confidenceScore} | dbLatency=${dbLatency}ms`
      );
   ...
   */
  async saveCompleteSearchAndAnalysis(dto: SaveCompleteSearchAndAnalysisDto): Promise<any> {
    if (!dto.searchQuery || !dto.searchQuery.trim()) {
      throw new BadRequestError('Search query cannot be empty');
    }
    ...
    try {
      ...
      const dbLatency = Date.now() - startTime;
      console.log(
        `[HistoryService] Search & Analysis saved atomically | id=${record.id} | query="${record.searchQuery}" | patentsCount=${dto.retrievedPatents?.length ?? 0} | hasAnalysis=${!!dto.noveltyAnalysis} | dbLatency=${dbLatency}ms`
      );
   ...
   */
  async getHistoryById(id: string): Promise<any> {
    if (!id || !id.trim()) {
   ...
   */
  async listHistory(filters: HistoryQueryFilterDto): Promise<HistoryListResponseDto> {
    const startTime = Date.now();
    ...
    console.log(
      `[HistoryService] History retrieval | page=${page} | limit=${limit} | count=${items.length} | totalItems=${totalItems} | dbLatency=${dbLatency}ms`
    );
   ...
   */
  async findReusableAnalysis(searchQuery: string, appliedFilters?: Record<string, any> | null): Promise<any | null> {

...
export class HistoryController {
  ...
  async getHistory(request: FastifyRequest, reply: FastifyReply): Promise<void> {
    ...
    request.log.info(
      `[HistoryAPI] GET /api/history | page=${response.meta.page} | limit=${response.meta.limit} | totalItems=${response.meta.totalItems}`
    );
   ...
   */
  async getHistoryById(request: FastifyRequest, reply: FastifyReply): Promise<void> {
   ...
   */
  async deleteHistory(request: FastifyRequest, reply: FastifyReply): Promise<void> {
    const parseResult = HistoryParamIdDtoSchema.safeParse(request.params);

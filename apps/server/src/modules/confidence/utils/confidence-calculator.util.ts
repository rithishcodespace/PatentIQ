...
export class ConfidenceCalculatorUtil {
   ...
   * 0-39:   Very Low
   */
  public static mapScoreToLevel(score: number): ConfidenceLevel {
   ...
   */
  public static calculateMetadataCompleteness(patents: SearchResult[]): number {
    if (!patents || patents.length === 0) return 0;

    let totalFields = 0;
    let filledFields = 0;

    const fieldsToCheck: (keyof SearchResult)[] = [
      'patentId',
      'title',
      'abstract',
      'ipc',
   ...
   */
  public static calculateScoreDistribution(scores: number[]): number {
   ...
   */
  public static calculateAnalysisCompleteness(novelty: NoveltyAnalysisResult): number {
    if (!novelty) return 0;

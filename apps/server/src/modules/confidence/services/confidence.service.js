...
export class ConfidenceService {
    ...
    constructor(customConfig) {
        this.config = {
            ...
            distributionWeight: parseFloat(process.env.CONFIDENCE_DISTRIBUTION_WEIGHT || process.env.CONFIDENCE_OVERLAP_WEIGHT || '0.15'),
     ...
     */
    calculateRetrievalConfidence(results, requestedTopK = 10) {
        try {
            ...
            const rawScore = topScoreScaled * this.config.topScoreWeight +
            ...
            console.log(`[ConfidenceService] Retrieval Confidence Calculated | score=${finalScore} | level=${ConfidenceCalculatorUtil.mapScoreToLevel(finalScore)} | factors=${JSON.stringify(factors)}`);
            return {
                score: finalScore,
    ...
    calculateAnalysisConfidence(params) {
        try {
            ...
            const totalOverlaps = overlappingClaimsCount !== undefined
            ...
            const rawScore = retrievalConfidence * this.config.retrievalWeight +
            ...
            console.log(`[ConfidenceService] Analysis Confidence Calculated | score=${finalScore} | level=${ConfidenceCalculatorUtil.mapScoreToLevel(finalScore)} | factors=${JSON.stringify(factors)}`);
            return {
                score: finalScore,
                level: ConfidenceCalculatorUtil.mapScoreToLevel(finalScore),
            };
     ...
     */
    calculateOverallConfidence(retrievalScore, analysisScore) {
        try {
            const rawScore = retrievalScore * this.config.overallRetrievalWeight +
                analysisScore * this.config.overallAnalysisWeight;
            const finalScore = ConfidenceCalculatorUtil.clampScore(rawScore);
            console.log(`[ConfidenceService] Overall Confidence Calculated | score=${finalScore} | level=${ConfidenceCalculatorUtil.mapScoreToLevel(finalScore)} | retrievalScore=${retrievalScore} | analysisScore=${analysisScore}`);
            return {
                score: finalScore,
                level: ConfidenceCalculatorUtil.mapScoreToLevel(finalScore),
            };
     ...
     */
    computeFullConfidence(results, noveltyAnalysis, requestedTopK = 10, overlappingClaimsCount) {

...
import { ALLOWED_MIME_TYPES, getFileExtension, MIME_TO_EXTENSION, } from '../utils/upload.utils.js';
export class UploadValidator {
    ...
    static validate(file) {
        ...
        if (buffer.length > maxSizeBytes) {
            throw new BadRequestError(`File size exceeds maximum allowed limit of ${maxMB} MB (${buffer.length} bytes uploaded).`);
        }
        // 4. MIME type validation
        if (!ALLOWED_MIME_TYPES.includes(mimetype)) {
            throw new BadRequestError(`Unsupported MIME type '${mimetype}'. Allowed types: PDF (application/pdf), DOCX (application/vnd.openxmlformats-officedocument.wordprocessingml.document), TXT (text/plain).`);
        }
        // 5. Extension validation
        const extension = getFileExtension(filename);
        const expectedExtension = MIME_TO_EXTENSION[mimetype];
        if (!extension || extension !== expectedExtension) {
            throw new BadRequestError(`Invalid file extension '.${extension}' for MIME type '${mimetype}'. Expected '.${expectedExtension}'.`);
        }
        return {
            filename,
            mimetype,

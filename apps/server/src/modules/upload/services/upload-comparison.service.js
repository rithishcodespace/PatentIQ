...
export class UploadComparisonService {
    uploadService;
    ...
    ragService;
    historyService;
    constructor(uploadService, documentProcessorService, embeddingsService, searchService, ragService, historyService) {
    ...
    async compareDocument(dto) {
        const totalStart = Date.now();
        const topK = dto.topK && dto.topK > 0 ? dto.topK : 10;
        let targetDoc = dto.document;
        ...
        const matches = retrievedPatents.map((p, idx) => {
            const overlapMatch = overlapItems.find((item) => item.patentId === p.patentId);
            const matchingSections = overlapMatch?.relevantSections
                ? overlapMatch.relevantSections.map((s) => typeof s === 'string' ? s : s.section)
                : ['Abstract', 'Claims'];
            return {
                rank: idx + 1,
                patentId: p.patentId,
        ...
        console.log(`[UploadComparisonService] Comparison pipeline completed successfully | document="${targetDoc.title}" | matchesCount=${matches.length} | extractionMs=${extractionTimeMs}ms | embeddingMs=${embeddingTimeMs}ms | retrievalMs=${retrievalTimeMs}ms | ragMs=${ragTimeMs}ms | totalMs=${totalComparisonTimeMs}ms`);
        return {
            success: true,
            document: {
...
//# sourceMappingURL=upload-comparison.service.js.map
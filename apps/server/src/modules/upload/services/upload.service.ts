...
export class UploadService implements IUploadService {
  ...
  async uploadDocument(fileInput: UploadFileInput): Promise<UploadedDocumentRecord> {
    const filenameForLog = fileInput?.filename || 'unknown';
    const mimetypeForLog = fileInput?.mimetype || 'unknown';
    console.log(
      `[UploadService] Upload started for file '${filenameForLog}' (MIME: ${mimetypeForLog})`
    );
    ...
    try {
      ...
      console.error(
        `[UploadService] Upload validation failed for '${filenameForLog}': ${error.message}`
      );
      throw error;
    }
    ...
    try {
      ...
      console.log(
        `[UploadService] Upload completed successfully. Doc ID: ${documentRecord.id}, Size: ${size} bytes, MIME: ${mimetype}`
      );
  ...
  async getMetadata(id: string): Promise<UploadedDocumentRecord> {
  ...
  async deleteDocument(id: string): Promise<boolean> {
    ...
    try {
      ...
      if (err.code !== 'ENOENT') {
        console.warn(
          `[UploadService] Physical file deletion encountered non-fatal error: ${err.message}`
        );
      } else {
        console.warn(
          `[UploadService] Physical file not found on disk during deletion: ${document.storagePath}`
        );
      }
    }

...
4. Output raw JSON only. Do NOT include markdown code blocks or explanations outside the JSON.`;
     ...
     */
    static buildPrompt(inventionText) {
...
Output raw JSON only.`;
     ...
     */
    static parseDeconstructionResponse(llmOutput, rawInventionText) {
        ...
        try {
            ...
            if (parsed && typeof parsed === 'object') {
                const coreTitle = String(parsed.coreTitle || parsed.title || this.extractFirstSentence(rawInventionText) || 'Invention Disclosure').trim();
                const technicalDomain = this.toStringArray(parsed.technicalDomain || parsed.ipcClassifications, ['G06F 16/30']);
                let extractedFeatures = [];
                if (Array.isArray(parsed.extractedFeatures) && parsed.extractedFeatures.length > 0) {
                    extractedFeatures = parsed.extractedFeatures.map((item, index) => {
                        const importanceVal = String(item.importance || 'HIGH').toUpperCase();
                        const validImportance = importanceVal === 'CRITICAL' || importanceVal === 'HIGH' || importanceVal === 'MEDIUM' || importanceVal === 'LOW'
                            ? importanceVal
                            : 'HIGH';
                        return {
     ...
     */
    static createFallbackResult(rawInventionText) {
        ...
        const coreTitle = firstSentence.length > 60
    ...
    static toStringArray(input, fallback) {
...
//# sourceMappingURL=feature-deconstruction.prompt.js.map
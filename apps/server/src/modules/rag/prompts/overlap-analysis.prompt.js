...
Do not include markdown code wrappers (\`\`\`json). Output raw JSON array only.`;
     ...
     */
    static parseOverlapAnalysisResponse(llmOutput, retrievedPatents) {
        ...
        try {
            ...
            if (itemsArray.length > 0) {
                return itemsArray.map((item, idx) => {
                    ...
                    const similarityScore = typeof item.similarityScore === 'number'
                        ? item.similarityScore
                        : matchingPatent?.score ?? 0.85;
                    // Parse relevantSections
                    let relevantSections = [];
                    if (Array.isArray(item.relevantSections)) {
                        relevantSections = item.relevantSections.map((sec) => ({
                    ...
                    if (Array.isArray(item.overlappingClaims)) {
                        overlappingClaims = item.overlappingClaims.map((c) => {
                            const strengthRaw = String(c.overlapStrength || 'Medium');
                            const overlapStrength = strengthRaw === 'High' || strengthRaw === 'Low' ? strengthRaw : 'Medium';
                            const claimObj = {
     ...
     */
    static createFallbackOverlapItems(retrievedPatents) {
...
//# sourceMappingURL=overlap-analysis.prompt.js.map
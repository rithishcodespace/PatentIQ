...
export class OverlapAnalysisService implements IOverlapAnalysisService {
  constructor(
  ...
  async analyzeOverlap(
    ...
    let patents: SearchResult[] = searchResults || [];
    ...
    console.log(
      `[OverlapAnalysisService] Section & Claim Overlap Analysis completed | analyzedPatents=${patents.length} | overlappingClaimsCount=${overlappingClaimsCount} | promptMs=${promptTimeMs}ms | llmMs=${llmInferenceTimeMs}ms | totalMs=${totalOverlapTimeMs}ms`
    );

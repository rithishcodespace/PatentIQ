...
async function runRagApiIntegrationTests() {
  ...
  assert(
    systemPrompt.includes('STRICT GROUNDING & ANTI-HALLUCINATION RULES') &&
      systemPrompt.includes('NEVER fabricate or invent claim numbers'),
    'OverlapPromptBuilder: System prompt contains strict anti-hallucination rules'
  );
  ...
  const builtPrompt = OverlapAnalysisPromptBuilder.buildPrompt(
    'An autonomous drone charging station using wireless inductive charging',
    mockPatents,
    { maxClaimsLength: 50 }
  );
    ...
    if (resValid.statusCode === 200) {
      ...
      if (validBody.overlapAnalysis.length > 0) {
        ...
        if (item.overlappingClaims.length > 0) {
          assert(
            ['High', 'Medium', 'Low'].includes(item.overlappingClaims[0].overlapStrength),
            'Overlapping claim classifies strength as High, Medium, or Low'
          );
        }
      }

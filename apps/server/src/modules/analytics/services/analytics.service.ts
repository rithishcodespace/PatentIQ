...
export class AnalyticsService implements IAnalyticsService {
  ...
  private static readonly CACHE_KEY = 'analytics:overview';
  ...
  async getOverviewMetrics(): Promise<AnalyticsOverviewDto> {
    ...
    try {
      const cachedMetrics = await this.cacheProvider.get<AnalyticsOverviewDto>(AnalyticsService.CACHE_KEY);
      if (cachedMetrics) {
    ...
    const ipcCountMap = new Map<string, number>();
    ...
    const overviewMetrics: AnalyticsOverviewDto = {

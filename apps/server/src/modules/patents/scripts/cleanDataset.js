    ...
    async cleanDataset(options) {
        ...
        try {
            for await (const rawRow of parser) {
                ...
                const cleanedRow = PatentDatasetCleaner.sanitizeRow(rawRow);
                ...
                const patentIdKey = cleanedRow.patnum ||
                    cleanedRow.appnum ||
                ...
                if (!canContinue) {
                    await new Promise((resolve) => csvTransformStream.once('drain', resolve));
                }
                // Periodic Logging
                if (totalRead % logInterval === 0) {
                    const memoryUsageMB = (process.memoryUsage().heapUsed / 1024 / 1024).toFixed(2);
                    const elapsedSec = ((Date.now() - startTime) / 1000).toFixed(1);
                    Logger.info(`Processed ${totalRead.toLocaleString()} rows | ` +

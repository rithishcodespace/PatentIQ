...
export class PatentSearchBenchmarker {
     ...
     */
    async runBenchmark(queries, topK = 100) {
        ...
        for (let i = 0; i < queries.length; i++) {
            ...
            try {
                ...
                const highestSimilarityScore = results.length > 0 ? Math.max(...results.map((r) => r.score)) : 0;
                queryResults.push({
                    query,
        ...
        const queriesPerSecond = durationSeconds > 0 ? parseFloat((successfulCount / durationSeconds).toFixed(2)) : 0;
        const summary = {
...
//# sourceMappingURL=benchmarkSearch.js.map
...
export async function runPatentIngestionScript(options: IngestionOptions = {}): Promise<IngestionStats> {
    ...
    try {
      ...
      if (generateEmbeddings && patentSection.abstract) {
        try {
          ...
          Logger.warn(`Vector embedding generation skipped for ${fileName}: ${(embedErr as Error).message}`);
        }
      }

...
export async function searchRoutes(
  ...
  fastify.post('/', {
    schema: {
      ...
      description:
        'Executes real-time semantic vector similarity search against Pinecone index with optional metadata filters (IPC, country, date, owner). Automatically persists search queries and retrieved matches to PostgreSQL.',
    ...
    handler: (req: FastifyRequest, reply: FastifyReply) => controller.search(req, reply),
  });
  ...
  fastify.post('/benchmark', {
    schema: {
      ...
      description:
        'Executes retrieval benchmark evaluating Precision@K, Recall@K, MRR, NDCG, Hit Rate@K, and P95/P99 latency metrics across test queries.',

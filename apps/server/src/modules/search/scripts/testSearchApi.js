...
async function runApiIntegrationTests() {
    ...
    try {
        ...
        assert(bodyMissing.message && bodyMissing.message.includes('query is required'), 'Validation: Error message specifies "query is required"', bodyMissing.message);
        ...
        assert(resEmptyQuery.statusCode === 400, 'Validation: Empty query string returns HTTP 400', `Got status ${resEmptyQuery.statusCode}`);
        ...
        assert(resTopKExceeded.statusCode === 400, 'Validation: topK > 100 returns HTTP 400', `Got status ${resTopKExceeded.statusCode}`);
        const bodyTopKExceeded = JSON.parse(resTopKExceeded.payload);
        assert(bodyTopKExceeded.message && bodyTopKExceeded.message.includes('maximum topK is 100'), 'Validation: Error message specifies "maximum topK is 100"', bodyTopKExceeded.message);
        ...
        assert(resTopKLow.statusCode === 400, 'Validation: topK < 1 returns HTTP 400', `Got status ${resTopKLow.statusCode}`);
        ...
        assert(resInvalidSection.statusCode === 400, 'Validation: Invalid section enum returns HTTP 400', `Got status ${resInvalidSection.statusCode}`);
        ...
        assert(resInvalidDate.statusCode === 400, 'Validation: Invalid ISO date string returns HTTP 400', `Got status ${resInvalidDate.statusCode}`);
        ...
        assert(resInvalidDateRange.statusCode === 400, 'Validation: publicationDateFrom > publicationDateTo returns HTTP 400', `Got status ${resInvalidDateRange.statusCode}`);
...
//# sourceMappingURL=testSearchApi.js.map
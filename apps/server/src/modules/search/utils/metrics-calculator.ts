...
export class MetricsCalculator {
  ...
  public static generateRecommendations(
    ...
    recommendations.push(
      `Embedding generation accounts for ${embeddingRatio.toFixed(1)}% of total search latency (${summary.averageEmbeddingTime}ms avg).`
    );

    if (embeddingRatio > 40) {
      recommendations.push(
        'High embedding overhead detected. Consider batching embedding requests or deploying a dedicated local GPU embedding service.'
      );
    }

    // 2. Vector Search Latency Analysis
    recommendations.push(
      `Pinecone query latency averages ${summary.averageSearchTime}ms.`
    );

    if (summary.averageSearchTime > 500) {
      recommendations.push(
        'Pinecone query latency exceeds 500ms. Consider reducing requested topK or evaluating index pod capacity.'
      );
    }
    ...
    if (summary.p95Latency > summary.averageLatency * 1.8) {
      recommendations.push(
        `Tail latency spike detected: P95 latency (${summary.p95Latency}ms) is significantly higher than average (${summary.averageLatency}ms). Review network connection stability to Pinecone.`
      );
    }
    ...
    if (!cacheEnabled) {
      ...
      recommendations.push(
        'Query caching is active. Repeated benchmark queries benefit from sub-millisecond retrieval.'
      );
    }
    ...
    if (quality.precisionAt10 !== null) {
      if (quality.precisionAt10 < 0.5) {
        recommendations.push(
          `Precision@K is low (${quality.precisionAt10}). Consider tuning hybrid weights (BM25 + Semantic) or cross-encoder reranking.`
        );
      } else {
        recommendations.push(
          `Retrieval Quality: High Precision@K (${quality.precisionAt10}) and NDCG (${quality.ndcg}).`
        );
      }
    }
   ...
   */
  public static percentile(sortedNumbers: number[], p: number): number {

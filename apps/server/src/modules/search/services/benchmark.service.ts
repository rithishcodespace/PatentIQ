...
export class BenchmarkService implements IBenchmarkService {
   ...
   */
  async runBenchmark(request: BenchmarkRequest): Promise<BenchmarkReport> {
    ...
    const collectedMetrics: SingleQueryMetric[] = [];
    ...
    for (let iter = 0; iter < iterations; iter++) {
      for (const queryText of queries) {
        ...
        const metric: SingleQueryMetric = {
          query: trimmedQuery,
    ...
    console.log(
      `[BenchmarkService] Benchmark Run Completed | totalRuns=${summary.queries} | iterations=${iterations} | averageLatency=${summary.averageLatency}ms | P95=${summary.p95Latency}ms | P99=${summary.p99Latency}ms | throughput=${summary.throughput} req/s | totalTime=${totalExecutionTimeMs}ms`
    );

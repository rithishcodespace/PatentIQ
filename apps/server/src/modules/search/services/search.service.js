...
import { BadRequestError, InternalServerError, ServiceUnavailableError, GatewayTimeoutError, } from '../../../common/errors/http-errors.js';
...
export class SearchService {
    ...
    confidenceService;
    cacheProvider;
    constructor(embeddingProvider, searchRepository, historyService, confidenceService, cacheProvider, bm25Service, rrfReranker) {
     ...
     * Measures Pinecone search latency.
     */
    async searchVectors(vector, topK = 10, filter) {
     ...
     */
    async executeSearch(queryText, topK = 100, filters) {
     ...
     */
    async search(input, topKParam, filtersParam) {
        const totalStart = Date.now();
        ...
        console.log(`[SearchService] Search execution completed | query="${trimmedQuery}" | topK=${topK} | count=${results.length} | highestScore=${highestScore} | latency=${metrics.totalExecutionTimeMs}ms`);
